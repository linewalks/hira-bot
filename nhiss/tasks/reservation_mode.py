...
from utils.message import convert_target_day, success_msg, failure_msg
...
def run_on_time(info, headless: bool = False, debug: bool = True):
  ...
  try:
    ...
    result = reservation_content_fill(bot, register_info['target_day'], register_info['is_register_am'], register_info['is_seoul'], check_date = False)
    ...
    # 2. 갱신된 날짜 테이블 내에서 선택
    bot.selectReservationDate(register_info['target_day'], register_info['is_register_am'], register_info['is_seoul'])
    ...
    print(err)
    send_message(failure_msg(register_info['user_name'], register_info['region'], target_day_msg))
  finally:
